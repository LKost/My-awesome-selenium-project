...
{
    public class CustomPageObjectDecorator : DefaultPageObjectMemberDecorator, IPageObjectMemberDecorator
    {
        private IWebDriver driver;
        public CustomPageObjectDecorator(IWebDriver driver) => this.driver = driver;
        ...
        public new object Decorate(MemberInfo member, IElementLocator locator)
        {
            ...
            else if (typeof(Element).IsAssignableFrom(targetType))
            {
                ...
                return GetElement(targetType, webElement, driver, property.Name);
            }
            ...
            else if (targetType.GetInterfaces().FirstOrDefault(t => t.IsGenericType && t.GetGenericTypeDefinition() == typeof(ICollection<>)) != null)
            {
                ...
                if (typeof(Element).IsAssignableFrom(elementOfListTargetType))
                {
                    var cache = ShouldCacheLookup(member);
